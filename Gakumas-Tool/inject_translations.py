...
def inject_translations(txt_path, xlsx_path, output_path):
    ...
    df["translated text"] = df["translated text"].apply(
        lambda x: x.replace("\n", "\\n")
    )

    # Create a list of tuples with original and translated texts and names, excluding empty 'translated text'
    translation_pairs = [
        (row["text"], row["translated text"], row["name"], row["translated name"])
        for _, row in df.iterrows()
        if row["translated text"].strip()
    ]
        ...
        for line in lines:
            ...
            if "[message" in original_line:
                for (
                    ...
                    original_name,
                    translated_name,
                ) in list(translation_pairs):
                    if (
                        f"name={original_name}" in original_line
                        and f"text={original_text}" in original_line
                        and translated_text != "nan"
                        and translated_name != "nan"
                    ):
                        original_line = original_line.replace(
                            f"name={original_name}", f"name={translated_name}"
                        )
                        original_line = original_line.replace(
                            f"text={original_text}", f"text={translated_text}"
                        )
                        translation_pairs.remove(
                            (
                                original_text,
                                translated_text,
                                original_name,
                ...
                for (
                    ...
                    if (
                        f"text={original_text}" in original_line
                        and translated_text != "nan"
                    ):
                        original_line = original_line.replace(
                            f"text={original_text}", f"text={translated_text}"
